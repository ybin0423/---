...
### **정렬 알고리즘**
  ...
  **5. 병합 정렬 (merge sort) - O(nlogn)**
    -> 데이터를 반으로 나누어 정렬을 수행
      - <ins>분할 정복; divide and conquer</ins>
  **6. 퀵 정렬 (quick sort)**
  -> 맨 왼쪽 요소 = 피벗 (pivot)
  - **<ins>왼쪽 마커</ins>** = 피벗 바로 오른쪽에 위치한 요소
  - **<ins>오른쪽 마커</ins>** = 배열 맨 오른쪽 요소

  **7. 힙 정렬 (haep sort) - O(nlogn)**
  -> 최댓값이 있는 노드를 바로 위 부모 노드와 바꿈

  - 시간 복잡도가 최신, 평균, 최악에 상관없이 항상 O(nlogn)을 유지.
    
  **8. 버킷 정렬 (bucket sort)**
  
   - 평균: O(n+ n^2/m + m) , m=버킷수
   - 최악: O(n^2)
    ...
    3. 버킷 안에서 정렬한 요소를버킷의 순서에 따라 나열해서 정렬을 완료함. 
...
![image](https://github.com/user-attachments/assets/95cd2b87-9210-46ac-854b-39498adb2fb5)

1. **<ins>너비 우선 탐색 (BFS)</ins>** - 그래프를 같은 층별로 탐색.
2. **<ins>깊이 우선 탐색 (DFS)</ins>** - 경로 하나의 모든 층을 탐색 후 다음 경로 탐색.
   - **백트래킹 (back tracking)** = 모든 경우를 다 탐색해 문제의 해답을 찾는 제어 알고리즘
   - **데이크스트라 알고리즘 (Dijkstra Algorithm)** = 그래프의 노드 하나에서 다른 노드까지의 최단 경로 찾는다.
   - 그래프의 가중치 = 비용
   - 블라인드 탐색 / 무정보 탐색 = 알고리즘이 탐색하는 공간에 대한 정보가 없는 경우.
   - **벨먼-포드 알고리즘 (Bellman-Ford)** = 그래프에서 최단 경로를 찾는데 사용되며 가중 그래프를 탐색한다. (가중치가 음수인 에지를 갖는 그래프에서도 동작.)

 <경로 탐색 기법의 두가지 유형>
    1. 탐욕적 접근법 (greedy) - 현재 관점에서 적합한 경로를 선택함. 
    2. 휴리스틱 접근법 (heuristic) - 확률론을 토대로  해결책을 제공. 
   - <ins>A* 알고리즘-(휴리스틱 + BFS(best first search))</ins>
-> 알고리즘의 요소를 결합하여 휴리스틱적 해결책을 제공 -> 그결과 전체 순회 비용이 가장 낮은 경로를 택.

**8. 군집화 알고리즘**
-> 븐류 시스템과 머신러닝 시스템에서 널리 사용됌.

- K-means 알고리즘은 데이터 구조의 특정 속성을 사용하여 데이터를 분류하는 **그래프 군집화** 에 사용됌.
- **<ins>군집화 (Clustering)</ins>** = 데이터를 분류하여 관련된 요소끼리 묶는 동작
